          ...
          <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
              @auth
                @if (Auth::User()->isAdmin())
                  <a href="{{ url('/dashboard') }}" class="text-sm text-white dark:text-gray-500  no-underline">Dashboard</a>
                @endif
              @else
                  <a href="{{ route('login') }}" class="text-sm text-white dark:text-gray-500 no-underline">Log in</a>

                  @if (Route::has('register'))
                      <a href="{{ route('register') }}" class="ml-4 text-sm text-white dark:text-gray-500 no-underline">Register</a>
                  @endif
              @endauth
          </div>
      @endif
